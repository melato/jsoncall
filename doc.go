...
Example: A server can be implemented like this:
...
Calling this server with curl:

	curl -i -X POST --data-binary '{"p1":"a","p2":2}' http://localhost:8080/A

Returns:
	{"result":"a:2"}

You can specify an interface to restrict the served methods to a subset of the receiver methods:
...
Example Go client, without using generated code:
...
The jsoncall/generate package provides code to generate or modify .json api descriptors.
